...
export class BillingAddressDialogueComponent implements OnInit {
  ...
  ngOnInit() {
    this.pattern = '^[1-9][0-9]{5}$';
    this.phone_number_pattern = '^[1-9][0-9]{9}$'
  ...
  public cities;
  getCities(){
    this.apiTalk.getData(Config.API_URL+Config.CITY_URL)
    .then(data=>{
      console.log(data)
  ...
  public pincodepattern = '^[1-9][0-9]{5}$';
  public phone_number = '^[1-9][0-9]{9}$'
  editAddress(passed){
    ...
    this.editAddressForm = new FormGroup({
      name: new FormControl(''),
      phone_number: new FormControl('', Validators.pattern(this.phone_number)),
      address_line1: new FormControl(''),
      address_line2: new FormControl(''),
      landmark: new FormControl(''),
      city: new FormControl(''),
